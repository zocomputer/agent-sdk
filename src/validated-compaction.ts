 ...
 *
 * eve compacts a long conversation by asking the turn model for a summary of
 * the older messages, then replacing them with it. That summarization is
 * unvalidated — nothing checks the summary against what it replaced, and the
 * Slipstream paper (arXiv:2605.08580) measures exactly this failure: unvalidated
 * compaction silently drops load-bearing facts (the task list, the set of
 * modified files, pending verification steps), and the agent degrades without
 * any error surfacing.
 *
 * {@link withValidatedCompaction} wraps the turn model with a facade that
 * intercepts eve's compaction call (detected by {@link COMPACTION_SENTINEL} on
 * the system message — eve's compaction is the only `doGenerate` traffic on the
 * turn model; turns stream), asks the same model to audit the fresh summary
 * against the original transcript, and appends any missing facts to the summary
 * as a {@link RECOVERED_CONTEXT_HEADER} section. Validation is fail-open: a
 * judge error, timeout, or malformed verdict leaves the original summary
 * untouched, and normal turn traffic (`doStream`) is a pure delegate.
 *
 * Why wrap the turn model instead of authoring `compaction.model`: eve's
 * `loadSourceBackedRuntimeModelReference` resolves an instance authored at
 * `compaction.model` back to the top-level turn model, so that seam silently
 * no-ops (see `design/upstream-asks.md`).
 */
import type {
  LanguageModelV4,
...
function extractTranscript(prompt: LanguageModelV4Prompt): string {
  const chunks: string[] = [];
...
): part is Extract<LanguageModelV4Content, { type: "text" }> {
