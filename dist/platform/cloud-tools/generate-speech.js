...
function readBrokerErrorMessage(value) {
  ...
  const error = isRecord4(value.error) ? value.error : value;
...
var GenerateSpeechInputSchema = z5.object({
  ...
  language: z5.string().trim().min(2).max(16).optional().describe("ISO language code or 'auto', when supported."),
  model: z5.string().trim().min(1).optional().describe("Speech model id; omit to use the default."),
  output_dir: OutputDirSchema.optional().describe("Relative state-file output directory."),
  speed: z5.number().min(0.25).max(4).optional().describe("Speaking speed, when supported."),
  style: z5.string().trim().min(1).max(500).optional().describe("Short delivery or expression instruction, when supported."),
  text: z5.string().trim().min(1).max(1e5).describe("Text to speak."),
  voice: z5.string().trim().min(1).max(100).optional().describe("Voice id from media_models.")
});
var EstimateSchema = z5.discriminatedUnion("confidence", [
  z5.object({ confidence: z5.literal("exact"), amountUsd: z5.number().nonnegative() }),
  z5.object({ confidence: z5.literal("range"), minUsd: z5.number().nonnegative(), maxUsd: z5.number().nonnegative() }),
  z5.object({ confidence: z5.literal("unknown") })
]);
var GenerateSpeechOutputSchema = z5.object({
  asset: z5.object({ type: z5.literal("state_asset"), declarationName: z5.string(), path: z5.string(), contentType: z5.string().optional(), bytes: z5.number().optional() }),
...
export {
  generateSpeechTool,
  generate_speech_default as default,
